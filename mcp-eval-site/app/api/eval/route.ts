...
async function setupOAuthFlow(serverUrl: string, baseUrl: string) {
  ...
  try {
    ...
    try {
      ...
      console.log("Resource metadata found:", resourceMetadata);
    } catch (metadataError) {
      console.log(
        "⚠️  OAuth protected resource metadata not available, continuing with direct OAuth flow"
      );
      // This is OK - many servers don't implement RFC 9728
    }
    ...
    console.log(
      "🔍 Discovering authorization server metadata from:",
      authServerUrl
    );
    ...
    if (!authServerMetadata) {
      throw new Error(
        "Unable to discover authorization server metadata at " + authServerUrl
      );
    }
    console.log("Auth server metadata discovered:", authServerMetadata);
...
async function performOAuthTokenExchange(
  ...
  try {
    // Try to discover OAuth metadata again (we need the auth server URL)
    let resourceMetadata;
    try {
      resourceMetadata = await discoverOAuthProtectedResourceMetadata(
        serverUrl
      ...
      console.log(
        "OAuth protected resource metadata not available, using server URL directly"
      );
    }
    const authServerUrl = resourceMetadata?.authorization_server || serverUrl;
    const authServerMetadata = await discoverAuthorizationServerMetadata(
...
[
  ...
  try {
    ...
    console.log(
      `🎉 SUCCESS! GPT-5 Mini generated ${scenarios.length} scenarios, using first ${limitedScenarios.length}`
    );
...
async function attemptConnection(
  ...
  try {
    console.log(
      "🔌 Attempting connection (this will trigger OAuth if needed)..."
    );
...
async function testMCPServerWithOAuth(
  ...
  try {
    console.log(
      "🔗 Starting OAuth MCP connection test with auth code:",
      authCode.substring(0, 10) + "..."
    );

    if (!clientInfo || !storedCodeVerifier) {
      throw new Error(
        "Missing OAuth state: clientInfo and codeVerifier are required for token exchange"
      );
    }

    // Use lower-level OAuth functions for token exchange
    const tokens = await performOAuthTokenExchange(
      serverUrl,
      authCode,
      clientInfo,
      storedCodeVerifier,
    ...
    tests.push({
      name: "OAuth MCP Connection",
      passed: false,
      message: `Failed to connect with OAuth: ${
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authCode) {
      ...
      console.log(
        "Using stored OAuth state - clientInfo:",
        !!clientInfo,
        "codeVerifier:",
        !!codeVerifier
      );

      const authenticatedTests = await testMCPServerWithOAuth(
        serverUrl,
        authCode,
        baseUrl,
        clientInfo,
    ...
    try {
      ...
      tests.push({
        name: "MCP Protocol Support",
        passed: false,
        message: `Could not verify MCP protocol: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
        duration: Date.now() - test2Start,
      });
    ...
    try {
      ...
      try {
        ...
        tests.push({
          name: "MCP Connection",
          passed: true,
          message: `Connected successfully without authentication. Found ${
        ...
        if (isUnauthorized) {
          console.log(
            "🔐 OAuth required (detected 401/Unauthorized), setting up authorization flow..."
          );
